...
int main()
{
  ...
  while (1) {
    ...
    while (recv(connectSocket, line, MAX_MSG, 0) > 0) {
      cout << "  --  " << line << "\n";
      // get line, retrive num from input as well as operation
      // call func calculator(num, operation) and
      // covert the calculation back into string datatype
      // Send converted line back to client.

    char *tokPtr; // user input line is a char datatype
    stack<double> num_stack; // container for input numbers
    int operatorRPN;
    double numA, numB, calcResult;
    char *next_item = NULL;
    
    tokPtr = strtok_r(line, " ", &next_item); // take input and store onto variable
    while(tokPtr) {
        operatorRPN = tokPtr[0]; // return 0 if stack is empty
        if (operatorRPN == '+' || operatorRPN == '-' || operatorRPN == '/' || operatorRPN == '*') { // check for operator
            if (num_stack.size() < 2) { // not enough operand present in the stack
                cout << "Error: too many operators." << endl;
                return -1;  
            }
            numB = num_stack.top(); num_stack.pop();
            numA = num_stack.top(); num_stack.pop();
            switch (operatorRPN) {
                case '+' : calcResult = numA + numB; break;
                case '-' : calcResult = numA - numB; break;
                case '/' : calcResult = numA / numB; break;
                case '*' : calcResult = numA * numB; break;
            }
            num_stack.push(calcResult);    
        }
        else {
            num_stack.push(atof(tokPtr));
        }
        tokPtr = strtok_r(nullptr,  " ", &next_item);
    }
